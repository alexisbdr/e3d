...
class EventEmulator(object):
    ...
    def _show(self, inp: np.ndarray, show="OpenCV"):
        min = np.min(inp)
    ...
    def generate_events(
        ...
        if self.show_input:
            if self.show_input == 'new_frame':
                self._show(new_frame, show="matplot")
            elif self.show_input == 'lpLogFrame':
                self._show(self.lpLogFrame1, show="matplot")
            elif self.show_input == 'diff_frame':
                self._show(diff_frame, show="matplot")
            else:
