...
def get_args():
    ...
    parser = argparse.ArgumentParser(description='EvUnet Training',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    
    parser.add_argument('-e', '--epochs', metavar='E', type=int, default=Params.epochs,
                        help='Number of epochs', dest='epochs')
    parser.add_argument('-b', '--batch-size', metavar='B', type=int, nargs='?', default=Params.batch_size,
                        help='Batch size', dest='batch_size')
    parser.add_argument('-l', '--learning-rate', metavar='LR', type=float, nargs='?',
                        default=Params.learning_rate,
                        help='Learning rate', dest='learning_rate')
    parser.add_argument('-f', '--load', dest='model_cpt', type=str, default=Params.model_cpt,
                        help='Load model from a .pth file')
    parser.add_argument('--img_size', dest='img_size', type=float, default=Params.img_size,
                        help='Downscaling factor of the images')
    parser.add_argument('-v', '--validation', dest='val_split', type=float, default=Params.val_split,
                        help='Percent of the data that is used as validation (0-100)')
    parser.add_argument('-cfg', '--config', dest='config_file', type=str, default=Params.config_file,
...
def train(unet, params):
    ...
    if not params.train_dir:
        raise FileNotFoundError("Training directory has not been set using cli or params")
    ...
    writer = SummaryWriter(comment=f'LR_{params.learning_rate}_EPOCHS_{params.epochs}_BS_{params.batch_size}_IMGSIZE_{params.img_size}')
    ...
    for epoch in range(params.epochs):
        ...
        with tqdm(total=train_size, desc=f'Epoch {epoch+1}/{params.epochs}', unit='img') as pbar:
