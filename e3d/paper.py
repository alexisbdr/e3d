...
def plot_event_volume(events: np.array, path: str, scale: float=0.2,  show_grid: bool = True):
...
# event_path = "/data2/jiangjianping/Event/events.txt"
...
if __name__ == '__main__':
    ...
    for item in nums:
        if event.shape[0] > item:
            ...
            event = event[::skip]
        path = os.path.join(dir_path, f'{item}.png')
        plot_event_volume(event, path, scale)
