...
def stitch_and_split():
    ...
    for folder in sorted(os.listdir(backgrounds)):
        ...
        images = [cv2.resize(cv2.imread(join(path, p)), (128, 128)) for p in sorted(os.listdir(path))[0:24]]
        ...
        stitched = cv2.copyMakeBorder(orig_stitched, 10, 10, 10, 10,
        ...
        while curr_size < stitched.shape[1] - 128:
            ...
            split = cv2.addWeighted(split, 1.5, blur, -.5, 0)
